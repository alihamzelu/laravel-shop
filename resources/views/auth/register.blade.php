...
<body class="bg-[#0b0c10] text-gray-200 antialiased min-h-screen flex flex-col justify-between relative overflow-x-hidden"
    ...
    <main class="flex-grow flex items-center justify-center px-4 py-6 z-10">
        <div class="bg-[#12141c] border border-gray-800 rounded-xl max-w-md w-full p-6 sm:p-8 shadow-2xl shadow-sky-500/5">
            ...
            <form id="login-form" method="POST" action="{{ route('login') }}" x-show="activeTab === 'login'" class="space-y-4">
                ...
                <div>
                    <div class="flex justify-between items-center mb-2">
                        <label class="text-xs text-gray-400 font-semibold uppercase tracking-wider block">Password</label>
            ...
            <!-- 2. REGISTER FORM (with username & phone) -->
            <form id="signup-form" method="POST" action="{{ route('register') }}" x-show="activeTab === 'signup'" class="space-y-4" x-cloak>
                ...
                <!-- Username -->
                <div>
                    <label for="reg_username" class="text-xs text-gray-400 font-semibold uppercase tracking-wider block mb-2">Username</label>
                    <input id="reg_username" type="text" name="username" value="{{ old('username') }}" required autocomplete="username"
                           class="w-full bg-[#0b0c10] text-white text-sm px-4 py-3 rounded border {{ $errors->has('username') ? 'border-red-500' : 'border-gray-800' }} focus:outline-none focus:border-sky-500 transition">
                    @error('username')
                        <span class="text-red-400 text-xs mt-1 block">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <!-- Phone Number -->
                <div>
                    <label for="reg_phone" class="text-xs text-gray-400 font-semibold uppercase tracking-wider block mb-2">Phone Number</label>
                    <input id="reg_phone" type="tel" name="phone" value="{{ old('phone') }}" required autocomplete="tel"
                           class="w-full bg-[#0b0c10] text-white text-sm px-4 py-3 rounded border {{ $errors->has('phone') ? 'border-red-500' : 'border-gray-800' }} focus:outline-none focus:border-sky-500 transition"
